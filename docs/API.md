...
H = skin_mpe(800, 1e-6)      # µm input also works
```
...
Apply the 2.5× de-rating for UV (280–400 nm) on successive days.
...
These legacy functions are available in `laser_mpe.legacy` for backward compatibility.

**Note:** Band-specific functions take wavelength in **micrometers**. The primary function `skin_mpe` accepts wavelength in nm.
