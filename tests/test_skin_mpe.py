...
Test organization:
    ...
    - TestUltraShortReturnsNan: Confirms t < 1e-9 returns nan (Standard range starts at 1e-9)
    - TestRepetitivePulse: Rules 1 and 2 per the standard
...
class TestUVBand:
    ...
    def test_thermal(self):
        ...
        from laser_mpe.legacy import H_skin_ICNIRP_UV_thermal
    ...
    def test_photochem_180_302(self):
        ...
        from laser_mpe.legacy import H_skin_ICNIRP_UV_photochemical
    ...
    def test_photochem_302_315(self):
        """Standard discrete 1-nm step values for 302-315 nm"""
        from laser_mpe.legacy import H_skin_ICNIRP_UV_photochemical
    ...
    def test_photochem_315_400(self):
        """Standard: H = 1.0 J/cm^2 constant for 10 s to 30 ks"""
        from laser_mpe.legacy import H_skin_ICNIRP_UV_photochemical
        assert_close(H_skin_ICNIRP_UV_photochemical(0.350, 100.0), 1.0)
        # Standard: H = 1.0 J/cm^2 constant for the entire 10 s to 30 ks range
    ...
    def test_wrapper_dispatches_uv(self):
        assert_close(skin_mpe(250, 1.0), 3.0e-3)
...
# =====================================================================
# 1400-1500 nm 5 and 7
# =====================================================================

class Test_1400_1500:
    """Verified against the standard tables 5 and 7."""
...
# =====================================================================
# 1500-1800 nm 5 and 7
# =====================================================================

class Test_1500_1800:
    """Verified against the standard."""
...
# =====================================================================
# 1800-2600 nm 5 and 7
# =====================================================================

class Test_1800_2600:
    """Verified against the standard."""
...
# =====================================================================
# 2600 nm - 1000 um 5 and 7
# =====================================================================

class Test_2600_1000um:
    """Verified against the standard."""
...
class TestBoundaryConvention:
    """
    Verify the Standard convention: t_1 <= t < t_2.
...
# =====================================================================
# Ultra-short pulses (t < 1e-9) return nan per the standard
# =====================================================================

class TestUltraShortReturnsNan:
    """Standard tables 5 and 7 start at 10^-9 s. All shorter durations return nan."""
...
class TestEdgeCases:
    """Out-of-range inputs, array inputs."""

    def test_wavelength_out_of_range(self):
        """Generic skin_mpe returns NaN for wavelengths outside the standard's range."""
        assert np.isnan(skin_mpe(100, 1.0)), "100 nm should return NaN"
        assert np.isnan(skin_mpe(2e6, 1.0)), "2000 um should return NaN"

    def test_array_input(self):
        from laser_mpe import skin_mpe_array
        H = skin_mpe_array(800, np.array([1e-8, 1e-6, 1e-3, 1.0]))
        assert H.shape == (4,)
