"""
Supplementary skin MPE parameters from ICNIRP 2013.

This module implements the supporting calculations needed for a complete
skin MPE evaluation:

    - T_max: Recommended maximum anticipated exposure durations (Table 4).
    - Limiting apertures for skin (Table 8).
    - Large area exposure correction for lambda > 1.4 um (ICNIRP 2013, Table 7 note c).
    - UV successive-day de-rating for 0.280-0.400 um (ICNIRP 2013).
    - Unit conversions between J/cm^2, mJ/cm^2, W/cm^2, mW/cm^2.

All values are verified against ICNIRP 2013.

References
----------
ICNIRP 2013, Table 4: Recommended Limiting Exposure Durations for
    CW and Repetitive-Pulse MPE Calculations (page 71).
ICNIRP 2013, Table 8: Limiting Apertures (Irradiance and Radiant
    Exposure) for Hazard Evaluation (page 78).
ICNIRP 2013, ICNIRP 2013, Table 7 note c: Large Area Exposures (page 67).
ICNIRP 2013, ICNIRP 2013: Repeated Exposures, Ultraviolet
    (0.180 to 0.400 um) -- Special Considerations (page 64).
"""
